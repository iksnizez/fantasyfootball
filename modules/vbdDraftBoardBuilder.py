...
class vbdDraftBoard():
    ...
    def get_database_data(self):
        ...
        # ==================================
        # historical draft data from my league
        ...
        self.dfProj = hf.query_database(
            """
            SELECT player.name, pos.pos AS pos, p.* 
            FROM projection p
        ...
        self.dfRank = hf.query_database(
            """
            SELECT player.name, pos.pos AS pos, o.outletName, a.analystName, r.* 
            FROM ranking r
        ...
        self.dfAdp= hf.query_database(
            """
            SELECT player.name, pos.pos AS pos, o.outletName, a.* 
            FROM adp a
        ...
        # player stats from prior season
        self.dfPlayers2024 = hf.query_database(
            """
            SELECT kp.playerId, kp.playerName, pos.pos AS pos, kp.adp, kp.points, kp.pointsAvg,  
    ...
    def get_replacement_player_score(self):
        ...
        # ---- flex logic adds to flex eligible position starter counts based on data
    ...
    def get_stat_aggregations(self):
        ...
        df_agg_ranking = self.dfRank.groupby(
            ['playerId', 'rankGroup'] # 'name',  'date'] 
        ).agg(
            ranking_mean=('ranking', 'mean'),
            ranking_std=('ranking', 'std')
        ).reset_index()
        """
        # Standardize the mean ranking within each position
        df_agg_ranking['ranking_mean_z'] = df_agg_ranking.groupby('rankGroup')['ranking_mean'].transform(
            lambda x: -((x - x.mean()) / x.std())
        )

        # Standardize the ranking volatility within each position
        df_agg_ranking['ranking_std_z'] = df_agg_ranking.groupby('rankGroup')['ranking_std'].transform(
            lambda x: -((x - x.mean()) / x.std())
        )

        # Combine into a single risk metric
        df_agg_ranking['ranking_risk'] = df_agg_ranking[['ranking_mean_z', 'ranking_std_z']].mean(axis=1)
        """
        ...
        df_agg_projection = self.dfProj.query('fantasyPoints > 0')

        # calculate agg projections
        df_agg_projection = df_agg_projection.groupby([
            'playerId', 'pos' #  'name', 'date' 
        ]).agg(
            projections_mean=('fantasyPoints', 'mean'),
            projections_std=('fantasyPoints', 'std')
        ).reset_index()
        """
        # Standardize the mean ranking within each position
        df_agg_projection['projections_mean_z'] = df_agg_projection.groupby('pos')['projections_mean'].transform(
            lambda x: (x - x.mean()) / x.std()
        )

        # Standardize the ranking volatility within each position
        df_agg_projection['projections_std_z'] = df_agg_projection.groupby('pos')['projections_std'].transform(
            lambda x: (x - x.mean()) / x.std()
        )

        # Combine into a single risk metric
        df_agg_projection['projections_risk'] = df_agg_projection[['projections_mean_z', 'projections_std_z']].mean(axis=1)
        """
        ...
        df_agg_adp = self.dfAdp.query('adp > 0')

        # calculate agg projections
        df_agg_adp = df_agg_adp.groupby(
            ['playerId'] #'name',  'date', 'outletName'] 
        ).agg(
            adp_mean=('adp', 'mean'),
            adp_std=('adp', 'std')
        ).reset_index()
        ...
        # =======================
        df_allstats_agg = df_agg_ranking[['playerId', 'ranking_mean', 'ranking_std']].merge(
            df_agg_projection[['playerId', 'projections_mean', 'projections_std']],
            on='playerId',
            how='left'
        ).merge(
        ...
        #df_allstats_agg['risk'] = df_allstats_agg[['projections_risk', 'ranking_risk']].mean(axis=1)

        col_order = [
            'name', 'pos', 'VBD_historical', 'VBD_VOLS', 'adp_mean', 'ranking_mean', 'projections_mean',  
            'ranking_std', 'projections_std',
        ...
        df_allstats_agg.rename(columns={
            'adp_mean':'adp', 'ranking_mean':'pos_ranking', 'projections_mean':'proj',  
            'ranking_std':'rank_std', 'projections_std':'proj_std',
